	...
	pusher := Pusher{
		...
		configAgent: &fakeConfigAgent{
			fakeConfigs: []config.PushGateway{
				// Initial wait
				config.PushGateway{Interval: 1 * time.Second},
				config.PushGateway{Interval: 2 * time.Minute},
				// This interval is smaller then the min allowed duration
				config.PushGateway{Interval: 0},
				config.PushGateway{Interval: 3 * time.Second},
				// This interval is bigger then the max allewd duration
				config.PushGateway{Interval: 30 * time.Hour},
				config.PushGateway{Interval: 1 * time.Second},
			},
		},
	}
	...
	pusher := Pusher{
		...
		configAgent: &fakeConfigAgent{
			fakeConfigs: []config.PushGateway{
				...
				// sleep intervall, subsequent calls will use both the intervall and
				// the endpoint
				config.PushGateway{Interval: minWaitDuration, Endpoint: "will be ignored"},
				config.PushGateway{Interval: minWaitDuration, Endpoint: ""},
				config.PushGateway{Interval: minWaitDuration, Endpoint: "endpoint one"},
				config.PushGateway{Interval: minWaitDuration, Endpoint: ""},
				config.PushGateway{Interval: minWaitDuration, Endpoint: ""},
				config.PushGateway{Interval: minWaitDuration, Endpoint: "endpoint two"},
				config.PushGateway{Interval: minWaitDuration, Endpoint: ""},
				config.PushGateway{Interval: minWaitDuration, Endpoint: "endpoint three"},
			},
		},
	}
	go pusher.Start("")
